...
vector< vector <float> > move(int dy, int dx, 
	...
	float blurring) 
{
	/*Where I used unsigned int type above for height and width, I now need to use signed int. This is to prevent errors in the arithmetic below when calculating the new indices*/
	int height = beliefs.size();
	int width = beliefs[0].size();
	vector < vector <float> > newGrid = zeros(height, width); /*I initialize the new belief grid first so I can modify values by index below*/

	int i, j, new_i, new_j;
	for (i = 0; i < height; ++i) {
		for (j = 0; j < width; ++j) {

			/*Vectors don't accept negative indices so this is the method I came up with for transforming any negative index into the corresponding positive index in a cyclical world*/
